...
| `ProfilePublisher` | `@linky/core/ProfilePublisher`                                                                                            | `ProfilePublishError` | Publishes own profile metadata (Nostr kind 0). Real Layer ships in core (`layerProfilePublisher`, #24): signs with the session's ACTIVE Nostr identity per publish; offline resolves successfully (event queued in `NostrPendingQueue`). Needs `RelayPool`, `NostrPendingQueue`, `Randomness`, `SecureStorage`.                        |
...
Relay list publishing (`relayLists.ts`, issue #23; `nostr.publish-relay-lists`): `publishRelayLists(activeIdentity, relaySettings)` signs and publishes the NIP-65 relay metadata (kind 10002, `["r", url]` tags, no markers) and the NIP-17 inbox relay list (kind 10050, `["relay", url]` tags) — shapes pinned against the PoC by `__fixtures__/relayLists.golden.json`. Replaceable events, so every trigger (relay-settings change, startup sync via `publishCurrentRelayLists`) just re-publishes; offline publishes fall back to `NostrPendingQueue` (`outcome: "queued"`, the shared `deliverNostrEvent` step) and go out on the next flush. `RelaySettings` is an input on purpose: today it comes from `CurrentEnvironment` (`currentRelaySettings`); #31's user-editable relay settings produce it instead.

Profile metadata (`profileMetadata.ts`, issue #24; `nostr.fetch-profile` / `nostr.publish-profile`): `publishProfileMetadata(activeIdentity, metadata)` signs and publishes the kind-0 profile (PoC content shape — `name`/`display_name`/`lud16`/`picture`/`image`, empties omitted — pinned by `__fixtures__/profileMetadata.golden.json`); same replaceable-event + offline-queue semantics as the relay lists. `fetchProfileMetadata(pubkeyHex)` loads a contact's kind-0 (newest `created_at` wins, malformed content tolerated) cache-first through `KeyValueStorage` (`linky.nostr.profileMetadata.v1:<pubkey>`, positive TTL 12 h / negative 2 min, best-effort — a broken store degrades to a refetch, never an error); `fetchProfilePictureUrl` extracts the displayable avatar URL. The relay query collects for a fixed window (PoC `querySync` parity) since the pool's `subscribe` is a live stream.

NIP-38 status (`profileStatus.ts`, issue #24; `profile.publish-status`): kind 30315 with `["d", "general"]`. `publishProfileGeneralStatus(activeIdentity, status)` publishes the raw status string (`null` clears with empty content); `fetchProfileGeneralStatus` returns the newest non-expired (NIP-40 `expiration`) general status, cached like metadata (`linky.nostr.profileStatus.v1:<pubkey>`). The Linky-specific payload is the currency-preference encoding consumed by post-v1 contact filters: `buildProfileGeneralStatus`/`parseProfileGeneralStatus` codec for `"<text>\n<BTC, CZK>"` (last line = unique known currency codes `BTC`/`CZK`/`USD`, case-insensitive; anything else is plain text) — exact PoC behavior, golden-pinned.
